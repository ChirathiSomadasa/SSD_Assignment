...
function Contact() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5001/contact/api')
  ...
  const handleRemove = async (id) => {
    if (window.confirm('Are you sure you want to remove this disease?')) {
      try {
        await axios.delete(`http://localhost:5001/contact/api/deleteContact/${id}`)
  ...
  const generateReport = () => {
    ...
    const dataToGenerate =
      filteredContacts.length > 0 ? filteredContacts : contactData
    ...
    img.onload = () => {
      ...
      const header = [
        ['Disease', 'Category', 'Description', 'Location', 'Solutions'],
      ]
  ...
  return (
    <div className='disease-list-container'>
      ...
      <div className='disease-filter-bar'>
        ...
        />
        <button className='disease-filter-search-btn' onClick={handleSearch}>
          Search
        </button>
        <button
          className='disease-filter-search-btn'
          onClick={handleClearSearch}
        >
          Clear Search
        </button>
        <button className='generate_report_disease' onClick={generateReport}>
          Generate Report
        </button>
      </div>

      <div style={{ overflowX: 'auto' }}>
        <table className='disease-table' style={{ minWidth: 900, width: '100%' }}>
          <thead>
            <tr>
            ...
            {contactData.length === 0 ? (
              ...
              contactData.map((contact) => (
                <tr key={contact._id}>
                  ...
                  <td style={{ textAlign: 'left' }}>{contact.description}</td>
